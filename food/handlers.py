...
def restaurants(bot, update):
    ...
    # Get page
    restaurant_page = all_restaurants.paginate(page=current_page, paginate_by=RESTAURANT_PER_PAGE)
...
def categories(bot, update):
    ...
    # Get page
    category_page = all_categories.paginate(page=current_page, paginate_by=CATEGORIES_PER_PAGE)
...
def category(bot, update):
    ...
    category_selected = int(update.callback_query.data.split("_")[1]) if update.callback_query else 1
    ...
    item_page = all_items.paginate(page=current_page, paginate_by=ITEMS_PER_PAGE)
    ...
    # If there is more than one page then navigation buttons are needed
    if total_pages > 1:
        keyboard.append(navigation_buttons("category_{}".format(category_selected), current_page, total_pages))
